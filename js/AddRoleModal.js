...
class AddRoleModal {
    ...
    render() {
                ...
                <div class="modal-content bg-white rounded-lg shadow-xl w-full max-w-6xl modal-dialog" style="width: 80%; max-width: 1000px;">
                    <div class="p-6 border-b border-gray-200">
                        <div class="flex items-center justify-between">
                            <h3 class="text-lg font-semibold text-gray-900">新增角色</h3>
                    ...
                    <form class="add-role-form p-6">
                        <!-- 角色名称和描述在同一排 -->
                        <div class="grid grid-cols-2 gap-4 mb-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">角色名称 <span class="text-red-500">*</span></label>
                                <input type="text" class="role-name w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="请输入角色名称" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">角色描述</label>
                                <input type="text" class="role-description w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="请输入角色描述">
                            </div>
                        </div>
                            ...
                            <div class="permission-cascade">
                                ...
                                <div class="cascade-level mb-6">
                                    ...
                                    <div class="platform-tabs flex space-x-4 border-b">
                                        <button type="button" class="tab-btn px-4 py-2 border-b-2 border-blue-600 text-blue-600 font-medium" data-platform="admin">总后台</button>
                                        <button type="button" class="tab-btn px-4 py-2 text-gray-500 hover:text-gray-700" data-platform="pc">PC中台</button>
                                        <button type="button" class="tab-btn px-4 py-2 text-gray-500 hover:text-gray-700" data-platform="mobile">小程序</button>
                                    </div>
                                </div>
        ...
        this.modalElement = this.options.container.querySelector('#addRoleModal');
    }
    ...
    bindEvents() {
        ...
        // 保存按钮事件
        this.modalElement.querySelector('.save-btn').addEventListener('click', () => this.saveRole());
    ...
    resetForm() {
        ...
        // 清空表单输入
        this.modalElement.querySelector('.role-name').value = '';
        this.modalElement.querySelector('.role-description').value = '';
    ...
    selectModule(moduleId) {
        ...
        // 更新模块列表选中状态
        this.modalElement.querySelectorAll('.module-list .cascade-item').forEach(item => {
    ...
    selectFunction(functionId) {
        ...
        // 更新功能列表选中状态
        this.modalElement.querySelectorAll('.function-list .cascade-item').forEach(item => {
    ...
    toggleModule(moduleId) {
        ...
        module.children.forEach(func => {
            if (func.children) {
                func.children.forEach(button => {
                    if (isSelected) {
                        this.state.selectedPermissions.delete(button.id);
                    } else {
                        this.state.selectedPermissions.add(button.id);
                    }
                });
            }
        });
    ...
    isModuleSelected(moduleId) {
        ...
        return module.children.every(func => {
            if (!func.children) return true;
            return func.children.every(button => this.state.selectedPermissions.has(button.id));
        });
    }
    ...
    isModulePartialSelected(moduleId) {
        ...
        module.children.forEach(func => {
            if (func.children) {
                func.children.forEach(button => {
                    if (this.state.selectedPermissions.has(button.id)) {
                        hasSelected = true;
                    } else {
                        hasUnselected = true;
                    }
     ...
     */
    saveRole() {
        const roleName = this.modalElement.querySelector('.role-name').value.trim();
        const roleDescription = this.modalElement.querySelector('.role-description').value.trim();
        ...
        const roleData = {
            ...
            platforms: [this.state.currentPlatform]
        };
        
        if (this.options.onSave) {
            this.options.onSave(roleData);
        }
    ...
    getSelectedPermissions() {
        return Array.from(this.state.selectedPermissions);
    }
